...
export function HeroBondCard() {
  ...
  return (
    <>
      <section className="relative overflow-hidden rounded-card bg-ink p-6 text-cream shadow-lift sm:p-9">
        ...
        />
        <PiggyMark
          className="pointer-events-none absolute -right-8 -bottom-12 h-56 w-56 text-cream/[0.05]"
          accent="transparent"
        />
        ...
        <div className="relative">
          ...
          <dl className="mt-8 grid max-w-lg grid-cols-2 gap-x-8 gap-y-5 sm:grid-cols-3">
            {stats.map((stat) => (
              <div key={stat.label}>
                <dt className="text-[11px] font-semibold uppercase tracking-[0.12em] text-cream/45">
                  {stat.label}
                </dt>
                <dd className="numeric mt-1.5 text-xl font-semibold sm:text-[22px]">
                  {ready ? stat.value : <Skeleton className="h-6 w-24 bg-cream/15" />}
                </dd>
              </div>
            ))}
          </dl>
          ...
          <div className="mt-9 max-w-2xl">
            <div className="flex items-baseline justify-between text-sm">
              <span className="numeric font-medium text-cream">
                {moneyExact(derived.totalRepaid)}{" "}
                <span className="font-normal text-cream/55">repaid</span>
              </span>
              <span className="numeric text-cream/55">{moneyExact(derived.balance)} to go</span>
            </div>
            <Progress
            ...
            />
            <p className="numeric mt-3 text-sm text-cream/55">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button
              size="lg"
              variant="ghost"
              className="border border-cream/25 text-cream hover:bg-cream/10"
